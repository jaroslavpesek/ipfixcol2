...
#ifndef PROTOBUF_KAFKA_COMPILED_KAFKAPRODUCER_HPP
#define PROTOBUF_KAFKA_COMPILED_KAFKAPRODUCER_HPP
...
public:
   ...
   */
  int produce(const char *data, size_t len,
              int32_t partition = RD_KAFKA_PARTITION_UA);

  /**
   * \brief Compute RSS partition from 5-tuple
   *
   * Uses symmetric hashing so both directions of a flow go to the same
   * partition.
   *
   ...
   */
  static int32_t computeRssPartition(const uint8_t *src_ip, size_t src_ip_len,
                                     const uint8_t *dst_ip, size_t dst_ip_len,
                                     uint16_t src_port, uint16_t dst_port,
                                     uint8_t protocol, int32_t partition_count);
...
private:
  ...
  using uniq_topic =
      std::unique_ptr<rd_kafka_topic_t, decltype(&rd_kafka_topic_destroy)>;
  ...
  struct ThreadContext {
    ...
    std::atomic<uint64_t>
        cnt_delivered;                ///< Successful deliveries (thread-safe)
...
#endif // PROTOBUF_KAFKA_COMPILED_KAFKAPRODUCER_HPP
