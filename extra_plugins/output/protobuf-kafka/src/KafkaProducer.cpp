...
KafkaProducer::KafkaProducer(const Config& cfg, ipx_ctx_t* ctx)
    : m_ctx(ctx)
{
    ...
    auto set_config = [&](const char* key, const char* value) {
        ...
        if (res != RD_KAFKA_CONF_OK) {
            ...
            throw std::runtime_error(
                std::string("Failed to set Kafka config '") + key + "'='" +
                value + "': " + err_str);
        }
    };
    ...
    if (!m_topic) {
        ...
        throw std::runtime_error(
            std::string("Failed to create Kafka topic: ") +
    ...
    if (pthread_create(&m_thread->thread, nullptr, &threadPolling, m_thread.get()) != 0) {
...
int
KafkaProducer::produce(const char* data, size_t len, int32_t partition)
{
...
void
KafkaProducer::produceError(struct timespec ts_now)
{
...
void*
KafkaProducer::threadPolling(void* context)
{
...
void
KafkaProducer::threadDeliveryCallback(rd_kafka_t* rk,
...
void
KafkaProducer::queryPartitionCount()
{
